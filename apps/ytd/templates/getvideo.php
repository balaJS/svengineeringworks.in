...
<?php } ?>
	...
	<p><b>Some age restrigcted videos you can't download form here. Aplogize for that.</b></p>
	<ul class="dl-list">
...
<?php if ($format['show_direct_url'] === true) { ?>
			<a class="btn btn-default btn-download" href="<?php echo $format['direct_url']; ?>" class="mime" target="_blank"><i class="glyphicon glyphicon-play-circle"></i> Play && get Video</a>
<?php } ?>
<?php if ($format['show_proxy_url'] === true) { ?>
			<a class="btn btn-primary btn-download hidden" href="<?php echo $format['proxy_url']; ?>" class="mime"><i class="glyphicon glyphicon-download-alt"></i> Download</a>
<?php } ?>
			<div class="label label-warning hidden"><?php echo $format['size']; ?></div>
			<div class="label label-default hidden"><?php echo $format['itag']; ?></div>
		</li>
<?php } ?>
	</ul>
	<hr />
...
<?php foreach($this->get('formats', []) as $format) { ?>
	<li>
		<a class="btn btn-default btn-type disabled" href="#"><?php echo $format['type']; ?> - <?php echo $format['quality']; ?></a>
<?php if ($format['show_direct_url'] === true) { ?>
		<a class="btn btn-default btn-download" href="<?php echo $format['direct_url']; ?>" class="mime" target="_blank"><i class="glyphicon glyphicon-play-circle"></i> Play here</a>
<?php } ?>
<?php if ($format['show_proxy_url'] === true) { ?>
		<a class="btn btn-primary btn-download hidden" href="<?php echo $format['proxy_url']; ?>" class="mime"><i class="glyphicon glyphicon-download-alt"></i> Download</a>
<?php } ?>
...
<?php } ?>
	<hr />
	<p class="hidden"><small>Note that you initiate download either by clicking "Direct" to download from the origin server or by clicking "Proxy" to use this server as proxy.</small></p>
