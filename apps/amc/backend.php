...
if(isset($_POST['amc-insert-btn'])){
	...
	$insert=mysqli_query($conn,"insert into customers (name,email,mobile,address,sending_date,amount) values('$name','$email','$mobile','$address','$sms_sending_date','$amount')");
...
if(isset($_POST['amc-update-btn'])){
	...
	$insert=mysqli_query($conn,"update customers set name='$name',email='$email',mobile='$mobile',address='$address',sending_date='$sms_sending_date',amount='$amount',status='$status' where id='$id' ");
	$insertID=mysqli_insert_id($conn);
